...
- **File:** `Complete/EOT.csv` (nightly all-history mirror; `@ReportLookbackDays = 0`, start bounded by the script's built-in 2021-08-01 floor)
...
**Business rules:**
- The nightly file is a full mirror: every verdict is re-checked every night
  and a row ARIA retracts disappears with it. No incremental assembly — the
  loader reads the single file (falling back to the retired Incremental/EOT
  windowed feed only if the mirror is absent).
...
- Loader derives `DocStatus` (Missing / Pending / Documented / Active) from
  `DocumentationStatus`.
- EOT notes don't exist in the warehouse before 2021-08-01 (the script's
  measured floor), so 2021-2023 MISSING rows are historical findings that can
  never resolve — waive them from the dashboard worklist.
